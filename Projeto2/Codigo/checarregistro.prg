...
IF lnResultadoCheck < 0
    MESSAGEBOX("Erro ao verificar registros existentes. Código de erro: " + TRANSFORM(lnResultadoCheck))
    RETURN
ENDIF
...
IF RECCOUNT("ResultadoCheck") > 0
    ...
    IF !EMPTY(cDthRent) AND EMPTY(cDthRSai)
        MESSAGEBOX("Registro encontrado com acesso ativo. Abrindo formulário para registrar saída.")
    ...
    ELSE
        IF EMPTY(cDthRent) AND EMPTY(cDthRSai)
            ...
            IF lnResultadoInsert < 0
                MESSAGEBOX("Erro ao criar novo registro. Código de erro: " + TRANSFORM(lnResultadoInsert) + CHR(13) + "Consulta SQL: " + lcSqlInsert)
                RETURN
            ENDIF
